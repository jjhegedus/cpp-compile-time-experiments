...
namespace TypeUtilities {
    ...
    namespace Impl {
        ...
        template <template <typename> class Predicate, typename T,
            typename... Ts>
            struct FilterImpl<Predicate, TypelistImpl<T, Ts...>>
        {
            using Next = typename FilterImpl<Predicate, TypelistImpl<Ts...>>::type;

            using type = std::conditional_t<
                Predicate<T>{},
                typename ConcatImpl<TypelistImpl<T>, Next>::type, 
                Next>;
        };

        //template<typename FunctionType, typename TupleType>
        //void ForTupleIf(FunctionType&& func, TupleType tuple, std::function<bool(void)> predicate) {
        //    std::apply(
        //        [func](auto&&... tupleItems) {
        //        ForEachArg(
        //            func,
